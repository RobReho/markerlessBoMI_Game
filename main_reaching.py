...
class MainApplication(tk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.check1.config(font=("Arial", self.font_size))
        self.check1.grid(row=4, column=1, padx=(0, 40), pady=30, sticky='w')

        # virtual keyboard checkbox
        self.check_keyboard = BooleanVar()
        self.check1 = Checkbutton(win, text="Virtual keyboard", variable=self.check_keyboard)
        self.check1.config(font=("Arial", self.font_size))
        self.check1.grid(row=4, column=2, padx=(0, 40), pady=30, sticky='w')
        
        # pick and place game checkbox
        self.check_game = BooleanVar()
        self.check1 = Checkbutton(win, text="Play Game", variable=self.check_game)
        self.check1.config(font=("Arial", self.font_size))
        self.check1.grid(row=4, column=3, padx=(0, 40), pady=30, sticky='w')
        #############################################################
...
def start_reaching(drPath, lbl_tgt, num_joints, joints, dr_mode, real_cursor,keyboard_box,game_box):
    """
    ...
    while not r.is_terminated:
        ...
        if not r.is_paused:
            ...
            if real_cursor:
                ...
                # After showing the cursor, check whether cursor is still
